const elementData = {
  ...
  "lanthanides": [
    ...
    {number: "59", symbol:"Pr", name:"Praseod,ymium", weight:"140.90765"},
    {number: "60", symbol:"Nd", name:"Neod,ymium", weight:"144.242"},
...
AFRAME.registerComponent("element-selection", {
  schema:{
    group: {type: 'string', default: ''}
  },

  init: function () {
    let menuLayout = this.el.parentNode;
    let el = this.el;
    let elementGroup = elementData[this.data.group];

    el.addEventListener('mouseenter', function () {
      while (menuLayout.firstChild) {
        menuLayout.removeChild(menuLayout.firstChild);
      }
      ...
      for (el of elementGroup) {
        ...
        elementPanel.setAttribute('element-text-info', {
          number: number,
          symbol: symbol,
          name: name,
          weight: weight
        ...
        menuLayout.appendChild(elementPanel);
      }
    });
  }
});
